...
class UnifiedManifoldMapper:
    ...
    def fit(self, X):
        ...
        print(f"🔧 Dimensionality reduction settings: dim_ratio={self.dim_ratio}, max_dim={self.max_dim} => Used dimensions: {no_dims}")
        ...
        for type_mapping in self.mapping_types:
            ...
            try:
                ...
                print(f"✅ Mapping successful: {type_mapping}, Mapped dimensions: {X_mapped.shape[1]}, Time elapsed: {end - start:.2f}s")
    ...
    def _init_model(self, type_mapping, n_samples, no_dims):
        no_dims = max(1, min(no_dims, n_samples - 1))  # Enforce safety bounds
        if type_mapping == 'PCA':
            ...
            if n_samples > self.max_samples_for_exact and self.use_nystroem:
                print(f"⚠️ Sample size {n_samples} exceeds threshold, using Nystroem approximated KPCA ({type_mapping})")
...
def neighborhood_Measure_mm(data, mapper=None, mode='normal', score_mode='normal'):
    ...
    if mode == 'raw':
        ...
        manifold = []
        for cls in classes:
            alpha = np.array([1.0])  # Only one mapping (Raw), weight is 1
