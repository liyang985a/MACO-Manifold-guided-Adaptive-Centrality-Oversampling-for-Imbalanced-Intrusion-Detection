...
from manifold_mapper import neighborhood_Measure_mm  # 依赖你上一个文件
...
def cen_mar_func_top_opp(data, global_features, global_labels,
    ...
    for i in range(r):
        ...
        if verbose and i < 1:
            ...
            print(f"    异类候选类别: {top_opp_classes}，样本数: {len(opp_indices_all) if 'opp_indices_all' in locals() else 0}，选中: {len(opp_indices)}")
...
def over_multi_manifold(data, global_data, mapper=None, min_class_size=5,
    ...
    if use_score_filter:
        ...
        print(f"🎯 使用 score filter: top {filter_ratio * 100:.0f}% ，保留 {len(sorted_weight)} 个样本")
...
def over_classify_mlp(data, train_mlp_once_func,
                      min_class_size=5, max_rounds=1, kf=2, iter_num=2,
                      mapping_mode='normal'):
            ...
            for round_num in range(1, max_rounds + 1):
                ...
                for cls in valid_classes:
                    ...
                    sorted_weight, sorted_data, sort_label = over_multi_manifold(
                        cls_data, d, mapping_mode=mapping_mode
                    )
    ...
    if final_new_samples:
        ...
        if final_new_samples_filtered:
            ...
            print("✅ 最终增强数据已保存为 final_multiclass_augmented6.csv")
...
def save_final_augmented_data(original_data, final_new_samples, filename="final_multiclass_augmented6.csv"):
    print("📦 正在保存最终增强训练集...")
...
def main():
    ...
    # =========================================================
    print("\n" + "=" * 50)
    print("⏱️ 开始运行时间性能评测 (SMOTE vs MACO)")
    print("=" * 50)
    print("⏳ 正在运行标准 SMOTE...")
    ...
    try:
        ...
        smote_total_time = end_time_smote - start_time_smote
        print(f"✅ SMOTE 运行完毕! 耗时: {smote_total_time:.2f} 秒")
        print(f"   生成后样本总数: {len(y_smote)}")
    except Exception as e:
        print(f"❌ SMOTE 运行失败: {e}")
        smote_total_time = 0.1  # 防止除0
    print("=" * 50 + "\n")
    ...
    # =========================================================
    print("\n🚀 执行多类过采样增强实验（MACO）...")
    start_time_maco = time.time()

    results = over_classify_mlp(train_data, train_mlp_once)

    end_time_maco = time.time()
    maco_total_time = end_time_maco - start_time_maco

    (after_Precision, after_Recall, after_F1, _, after_accuracy) = results

    print(f"\n✅ 交叉验证：多类过采样 MLP 平均指标（MACO）：")
    ...
    print("\n" + "=" * 50)
    print("📊 运行时间对比总结：")
    print(f"SMOTE 耗时: {smote_total_time:.2f} 秒")
    print(f"MACO  耗时: {maco_total_time:.2f} 秒")
    if smote_total_time > 0:
        print(f"MACO 的总计算开销大约是 SMOTE 的 {maco_total_time / smote_total_time:.1f} 倍")
        print("(注: MACO 包含了多流形映射、邻居打分、交叉验证和 MLP 分类器反馈循环的时间)")
    print("=" * 50 + "\n")

    # 6. 使用增强后的数据，在真实测试集上评估 MACO
    print("\n🚀 使用 MACO 增强数据重新训练，并在真实测试集上评估...")
    final_augmented_data = pd.read_csv("final_multiclass_augmented6.csv").values
