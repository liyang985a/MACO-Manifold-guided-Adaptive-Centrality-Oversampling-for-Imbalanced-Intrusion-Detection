...
from multimanifoldSMOTE3 import neighborhood_Measure_mm
...
def over_classify_mlp(
            ...
            kf=2,
            iter_num=2,
            use_score_filter=True,  # ✅ 新增：是否启用得分筛选
            filter_ratio=0.8  # ✅ 新增：得分筛选比例
    ):
    ...
    if final_new_samples:
        ...
        if final_new_samples_filtered:
            ...
            print("✅ 最终增强数据已保存为 final_multiclass_augmented3.csv")
...
def save_final_augmented_data(original_data, final_new_samples, filename="final_multiclass_augmented3.csv"):
    print("📦 正在保存最终增强训练集...")
...
def main():
    ...
    y_true, y_pred = train_mlp_once(X_aug, y_aug, X_test, y_test, num_classes, device_id=1)
